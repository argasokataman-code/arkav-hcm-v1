...
| `/tax-rates` | Tax Rates / tax governance shell | Tidak ada JS runtime khusus; view statis + modal template | Tidak ada API khusus; runtime pajak aktual tersebar di employee, salary components, dan payroll | **HCM Admin** (`hcm.web.admin`) | Tidak ada kontrak mutasi khusus untuk halaman ini | Harus diperlakukan sebagai shell legacy/non-authoritative sampai ada model data + API + wiring runtime resmi |
