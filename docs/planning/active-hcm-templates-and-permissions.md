...
Sejak 2026-05-17 (ghost cleanup appearance), route aktif + menu untuk `/appearance` disembunyikan total karena tidak ada consumer runtime yang memakai setting `appearance_*` di shell aplikasi aktif. Route tenant-admin ini tidak lagi didaftarkan pada web runtime dan group API `appearance` juga tidak lagi diterima oleh endpoint settings generik.
...
| `/roles-permissions` | Role & Permissions | `users-management.js` | `GET /v1/hcm/user-management/roles`, `GET /permissions`, `POST /roles/{id}/permissions:sync` | **HCM Admin** (`hcm.web.admin`) | **HCM Admin only** (`EnsuresHcmAdmin`); API filter: `module=system` permissions hanya muncul untuk **global HCM admin** | Web guard: `hcm.web.admin` (tenant admin accessible). API permission filter: katalog `module=system` hanya dikirim ke **global HCM admin** via backend logic; tenant admin tetap bisa akses halaman tetapi tidak melihat system permissions di dropdown builder role. Enforcement: server-side permission filtering, bukan UI-only. |
...
| `/invoice-settings` | Konfigurasi invoice billing platform | `invoice-settings-data.js` | `GET/PUT /v1/hcm/invoice-settings` | **Global HCM Admin** (`hcm.web.admin` + `requiresGlobalHcmAdmin()` override) | **Global HCM Admin only**; API RBAC via `settings.manage` permission + global-admin check | Mengatur prefix, due days, round-off, tax, header/footer terms untuk invoice billing yang diterbitkan platform ke tenant. Web guard: `hcm.web.admin` dengan override `requiresGlobalHcmAdmin()` di middleware `EnsureHcmWebAdminPage`. API guard: permission `settings.manage` + global-admin enforcement. Preview billing invoice: endpoint `/v1/saas/invoices/{id}/pdf` (global-admin-only). Menu sidebar hanya tampil untuk global admin. |
| `/email-settings` | Email Settings | `settings/email-settings.blade.php` display-only config table | `GET /email-settings`, `GET/PUT /v1/hcm/email-settings`, `POST /v1/hcm/email-settings/test-connection`, `GET /v1/hcm/email-settings/mailtrap-status` | **Global HCM Admin** (`hcm.web.global-admin`) | **Global HCM Admin only** (`ensureGlobalHcmAdmin`); all email settings endpoints platform-level | Halaman web + semua endpoint email settings runtime sekarang **global-only** (`ensureGlobalHcmAdmin`). Rationale: email provider config (Mailtrap, SMTP) adalah platform-level setting, bukan tenant-scoped. Web guard: `hcm.web.global-admin`. API guard: all endpoints enforce `ensureGlobalHcmAdmin()`. |
...
| `/geofences` | Geofence Settings | `geofence-management-data.js` + Leaflet (unpkg) | `GET/POST/PUT/DELETE /v1/hcm/geofences` | **HCM Admin** (`hcm.web.admin`) | **HCM Admin only** (`attendance.manage`) | Leaflet map CRUD; punch cek `GeofenceService` — reject 422 `GEOFENCE_VIOLATION` bila di luar |
...
| `/monthly-report` | Monthly Report payroll gabungan | `monthly-report-data.js` | `GET /payroll/monthly-report?periodYear&periodMonth`, `GET /payroll/monthly-report/export?periodYear&periodMonth&format` | **HCM Admin** (`hcm.web.admin`) | **hcmAdmin** | Satu laporan detail untuk payroll monthly + THR + PKWT per employee/periode, lengkap dengan export CSV/XLSX |
...
| `/termination` | Termination | `termination-data.js` | `GET/POST/PUT/DELETE /terminations`, `GET /terminations/settlement-preview`, `GET /terminations/{id}/settlement-preview`, `POST /terminations/{id}/clearance-items/{assignmentId}/return`, `POST/GET/PATCH/DELETE /terminations/{id}/checklist-items` | **HCM Admin** | Semua verb **hcmAdmin**; endpoint detail/per-user list tetap ownership-aware untuk self | Web: `hcm.web.admin` + `hcm.web.feature:termination`; API: `hcm.api.feature:termination` (standalone, bukan employee_lifecycle); paket: business, enterprise, ultimate, umkm, unlimited |
