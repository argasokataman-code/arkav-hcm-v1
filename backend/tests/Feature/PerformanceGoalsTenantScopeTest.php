...
#[IgnoreDeprecations]
class PerformanceGoalsTenantScopeTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_goals_scope_all_is_tenant_scoped_for_admin(): void
    {
        ...
        $goalA = PerformanceGoal::query()->create([
            'goal_type_id' => null,
            'user_id' => $userA->id,
        ...
        $goalB = PerformanceGoal::query()->create([
            'goal_type_id' => null,
            'user_id' => $userB->id,
            'manager_user_id' => null,
            'subject' => 'Goal B',
        ...
        $resA = $this->withHeader('Cookie', $cookieHeader)
            ...
            ->getJson('/v1/hcm/performance/goals?scope=all&perPage=50');

        $resA->assertOk()->assertJsonPath('success', true);
        $idsA = collect($resA->json('data'))->pluck('id')->all();
        $this->assertContains($goalA->id, $idsA);
        $this->assertNotContains($goalB->id, $idsA);
        ...
        $this->assertContains($goalB->id, $idsB);
