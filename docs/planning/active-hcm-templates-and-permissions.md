...
| `/teams` | Master team | `team-master-data.js` | `GET/POST/GET/PUT/DELETE /teams` | **HCM Admin** | Semua verb **hcmAdmin** (`employee.manage`) | Team lead assignment opsional via `team_lead_id` |
| `/teams/{id}/members` | Detail anggota per team | `team-master-data.js` (mode members) | `GET /teams/{id}/members` | Authenticated (deep-link) | **HCM Admin** atau **team lead team tersebut** | Akses API dibatasi ownership (`team_lead_id == user login`) |
