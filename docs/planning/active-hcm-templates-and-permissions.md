...
| `/salary-component-master` | ~~Master komponen gaji~~ — **REDIRECT** | — | — | — | — | Route dihapus sebagai halaman standalone; sekarang `redirect()->route('tax-employees.komponen-pajak')`. CRUD komponen gaji dipindahkan sepenuhnya ke `/tax-employees/komponen-pajak` (tab **Komponen Pajak** di Employee Tax & Payroll Compliance). |
| `/tax-employees` (termasuk `/tax-employees/komponen-pajak`) | Employee Tax & Payroll Compliance governance — **canonical entry point juga untuk CRUD komponen gaji** | `tax-governance-dashboard.js` | `GET/POST /hcm/tax-governance/**` + `GET/POST/PUT/DELETE /hcm/salary-components` + `GET /hcm/salary-component-categories` + endpoint pendukung profile/compliance | **HCM Admin** (`hcm.web.admin`) | **Tenant hcmAdmin** untuk view/runtime tenant, **tenant owner** untuk authoring policy sementara. | Canonical governance page sekaligus satu-satunya entry point untuk CRUD `hcm_salary_components`. Tab **komponen-pajak** menampilkan daftar komponen + tombol "Tambah Komponen", "Master Kategori", serta ikon edit/delete per baris. Merger dari `/salary-component-master` yang kini menjadi redirect. |
