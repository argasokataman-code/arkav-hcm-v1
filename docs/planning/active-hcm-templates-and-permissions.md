...
| `/employee-dashboard` | Dashboard karyawan | template | (bila di-wire ke summary) | **Employee/member tenant context** (`hcm.web.employee`) | Prefer **self** data | Owner/admin tenant di-redirect ke `/index` (2026-05-07) |
...
| `/tickets-employee` | Tickets (Employee) | `tickets-data.js` | `GET/POST /tickets`, `GET /tickets/{id}` | **Employee/member tenant context** (`hcm.web.employee:tickets-admin`) | Karyawan own scope; admin pakai jalur `/tickets-admin` | Owner/admin tenant otomatis diarahkan ke `/tickets-admin` (2026-05-07) |
...
| `/leaves-employee` | Cuti (karyawan) | `hcm-extras-data.js` | `leave-requests?scope=me` | **Employee/member tenant context** (`hcm.web.employee:leaves`) | **Self** untuk non-admin | Owner/admin tenant otomatis diarahkan ke `/leaves` (2026-05-07) |
...
| `/leave-type` | Katalog leave type per-company | inline script pada halaman `leave-type.blade.php` | `GET/POST/PUT/DELETE /leave-types` | **HCM Admin** (`hcm.web.admin` + feature `leave_management`) | Permission `leave.type` (semua verb); data di-scope per `company_id` | `hcm_leave_type_settings` sekarang per-company (`company_id`); dipakai juga oleh `GET /leave-type-options` |
...
| `/attendance-employee` | Absensi karyawan | `attendance-data.js` + **Leaflet** (unpkg) + OSM | `attendance/me/*` | **Employee/member tenant context** (`hcm.web.employee:attendance-admin` + feature `attendance`) | **Self** punch untuk non-admin; punch wajib GPS | Owner/admin tenant otomatis diarahkan ke `/attendance-admin` (2026-05-07) |
...
| `/overtime-employee` | Overtime (karyawan) | `hcm-extras-data.js` | `overtime-requests?scope=me`, `calculate` | **Employee/member tenant context** (`hcm.web.employee:overtime`) | Hanya data sendiri; kalkulator manual (tanpa dropdown karyawan) | Owner/admin tenant otomatis diarahkan ke `/overtime` (2026-05-07) |
| `/salary-component-master` | Master komponen gaji (payroll component registry) | `salary-component-master-data.js` | `GET/POST/PUT/DELETE /hcm/salary-components`, `GET/POST/PUT/DELETE /hcm/salary-component-categories` | **HCM Admin** (`hcm.web.admin`) | **hcmAdmin** | Halaman kembali standalone untuk kelola komponen payroll lintas domain (PPh21/BPJS/internal) dan tidak lagi menjadi tab di `/tax-employees`. |
| `/tax-employees` (plus subroute `/policies`, `/tenant-compliance`, `/employee-tax-profiles`, `/reports`) | PPh 21 Governance | `tax-governance-dashboard.js`, `tax-employee-profiles.js`, `tax-tenant-compliance.js` | `GET/POST /hcm/tax-governance/**` + endpoint pendukung employee profile/compliance | **HCM Admin** (`hcm.web.admin`) | **Tenant hcmAdmin** untuk view/runtime tenant, **tenant owner** untuk authoring policy sementara. | Scope dipersempit untuk domain PPh 21 dan compliance pajak; menu label diganti dari Tax Governance menjadi PPh 21 Governance agar tidak bercampur dengan BPJS. |
| `/bpjs-governance` (plus subroute `/policies`, `/employee-membership`, `/reports`) | BPJS Governance | `bpjs-governance-data.js` | `GET /hcm/bpjs-governance/reference`, `GET/POST/PUT /hcm/bpjs-governance/policies`, `GET/PUT /hcm/bpjs-governance/employee-membership`, `GET /hcm/bpjs-governance/reports` | **HCM Admin** (`hcm.web.admin`) | **hcmAdmin** | Domain terpisah dari PPh 21; fokus ke policy iuran BPJS, monitoring membership karyawan, dan checklist compliance BPJS dengan endpoint mandiri (tidak lewat salary component master). |
| `/employee-allowance-governance` (plus subroute `/policies`, `/assignments`, `/reports`) | Employee Allowance Governance | `employee-allowance-governance-data.js` | `GET /hcm/allowance-governance/reference`, `GET/POST/PATCH /hcm/allowance-governance/policies`, `POST /hcm/allowance-governance/policies/{policyRef}/activate`, `GET/POST/PATCH /hcm/allowance-governance/assignments`, `GET /hcm/allowance-governance/reports/compliance` | **HCM Admin** (`hcm.web.admin`) | Baseline runtime memakai `payroll.view` (read) + `payroll.manage` (mutasi); owner role default keluar dari payroll scope assignment/compliance | Domain tunjangan umum non-statutory; baseline default allowance Indonesia auto-seed dan tenant bebas override policy/assignment. |
| `/spt-masa-pph21`, `/spt-masa-pph21/{sptUuid}` | SPT Masa PPh 21 | `spt-masa-data.js` | `GET /v1/hcm/spt-masa/headers`, `POST /v1/hcm/spt-masa/headers`, `GET /v1/hcm/spt-masa/headers/{sptRef}`, `POST /v1/hcm/spt-masa/headers/{sptRef}/regenerate`, `POST /v1/hcm/spt-masa/headers/{sptRef}/mark-ready`, `POST /v1/hcm/spt-masa/headers/{sptRef}/submit`, `GET /v1/hcm/spt-masa/headers/{sptRef}/export.csv` | **HCM Admin** (`hcm.web.admin`) | `tax.spt.view` (list, show, export CSV); `tax.spt.manage` (generate, regenerate, mark-ready, submit) | Snapshot SPT Masa dari payroll yang sudah finalized; lifecycle: draft → ready → submitted; CSV export UTF-8 BOM untuk DJP. Karyawan tidak memiliki akses; regular employee: none. |
...
| `/document-center` | Document Center | `document-center.js` | `GET/POST/PUT/DELETE /v1/hcm/document-center/categories`, `GET/POST/PUT/DELETE /v1/hcm/document-center/documents`, `GET /documents/{id}/download` | **HCM Admin** atau user dengan `document_center.manage`/`document_center.view`; Karyawan self-service (tanpa permission doc) | Mutasi CRUD categories+documents hanya `document_center.manage`; lihat semua dokumen `document_center.view`; karyawan biasa hanya dokumen sendiri yang `employee_visible` | Feature-gated: `employee_document_center`; permission codes: `document_center.manage`, `document_center.view` |
