(function (window, document) {
    ...
    function renderDetail(ticket) {
        ...
        if (!main || !side) return;
        var employeeClosedLocked = ticket.status === "closed" && !state.canManageTickets;
        ...
        var closedLockNotice = employeeClosedLocked
            ? '<div class="alert alert-light border mt-3 mb-0">Ticket sudah ditutup. Employee tidak bisa menambah komentar atau attachment baru.</div>'
            : '';
        var commentForm = employeeClosedLocked
            ? ''
            : '<form data-ticket-comment-form class="mt-3"><textarea class="form-control mb-2" name="body" rows="3" maxlength="5000" required placeholder="Tulis komentar update ticket..."></textarea><button class="btn btn-primary btn-sm" type="submit">Tambah Komentar</button></form>';
        var attachmentForm = employeeClosedLocked
            ? ''
            : '<form data-ticket-attachment-form class="mt-3"><input type="file" name="file" class="form-control mb-2" required><button class="btn btn-outline-primary btn-sm" type="submit">Upload</button></form>';
        main.innerHTML = '<div class="card"><div class="card-header d-flex justify-content-between align-items-center"><div><h5 class="mb-1">' + esc(ticket.subject) + '</h5><div><span class="badge badge-soft-dark me-2">' + esc(ticket.code) + "</span>" + statusBadge(ticket.status) + '</div></div><div>' + prioBadge(ticket.priority) + '</div></div><div class="card-body"><p>' + esc(ticket.description || "") + '</p><hr><h6 id="komentar">Komentar</h6><div>' + comments + '</div>' + commentForm + '<hr><h6>Attachment</h6><div>' + atts + '</div>' + attachmentForm + closedLockNotice + '</div></div>';
