            ...
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
                    ...
                    <form method="GET" action="{{ url()->current() }}" class="d-flex align-items-center gap-2 flex-wrap">
                        <input type="search" name="q" class="form-control" style="min-width: 220px;" placeholder="Search code / regency / province" value="{{ $filters['q'] ?? '' }}">
                            ...
                            <tbody>
                                @forelse ($rows as $row)
                                    <tr>
                                        ...
                                        <td>{{ number_format((int) ($row->districts_count ?? 0)) }}</td>
                                        <td>{{ optional($row->updated_at)->format('d M Y H:i') ?? '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted py-4">No regencies synced yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
