...
| `/roles-permissions` | Role & Permissions | `users-management.js` | `GET /v1/hcm/user-management/roles`, `GET /permissions`, `POST /roles/{id}/permissions:sync` | **HCM Admin** | **HCM Admin only** (`EnsuresHcmAdmin`) | UI aktif untuk katalog role + preview permission per role + ringkasan blueprint permission di halaman + builder create/edit role dua panel dengan pencarian, preview, dan pilih permission langsung |
| `/saas/transactions` | Purchase Transactions | `purchase-transactions-data.js` | `GET/POST/PUT /transactions`, `GET /transactions/export` | **HCM Admin** | **HCM Admin only** (`isGlobalHcmAdmin`) | List/detail/export transaksi billing SaaS |
| `/saas/billing-overview` dan `/saas/billing-overview/invoices/{invoice}` | Trial & Billing Dashboard | `saas-billing-overview.js` | `GET /v1/saas/companies/billing-overview`, `GET /v1/saas/invoices/{invoice}`, `POST /v1/saas/invoices/{invoice}/send-email` | **HCM Admin** | **HCM Admin only** (`hcm.web.admin` + admin check di API) | Overview company trial/subscribed, badge mismatch state, halaman detail invoice terpisah, dan resend email invoice |
