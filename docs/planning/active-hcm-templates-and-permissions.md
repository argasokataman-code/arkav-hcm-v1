...
| `/saas/platform-tax` | Tax Reporting (SPT Platform) | `platform-tax.js` | `GET /v1/saas/tax/active-ppn-rate`, `GET /v1/saas/tax/dashboard`, `GET /v1/saas/tax/spt-ppn`, `GET /v1/saas/tax/spt-pph23`, `GET /v1/saas/tax/spt-pph-badan`, `GET /v1/saas/tax/spt-pph-badan/export` | **Global HCM Admin** (`hcm.web.global-admin`) | API SPT tetap global-admin; tarif PPN read-only dari compliance settings via `active-ppn-rate` | Fokus pelaporan (SPT PPN + SPT PPh23 + estimasi PPh Badan) dengan single source tarif PPN; policy tetap dikelola di menu compliance |
