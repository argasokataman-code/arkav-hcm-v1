...
class HcmPayrollRunController extends Controller
{
    ...
    public function startMockHostedCheckout(Request $request, int $id): JsonResponse
    {
        ...
        if ($selectedUserIds->isEmpty()) {
            return response()->json([
                ...
                'error' => [
                    'code' => 'PAYROLL_DISBURSE_NO_EMPLOYEES',
                    'message' => 'Tidak ada karyawan valid untuk diproses.',
                ],
            ], 422);
        }

        $companyId = $this->activeCompanyId($request);
        $runQuery = HcmPayrollRun::query()->whereKey($id);
        $this->applyTenantScope($runQuery, $companyId);
        $run = $runQuery->firstOrFail();
        ...
        $lines = HcmPayrollLine::query()
            ->with('user:id,name')
            ->where('hcm_payroll_run_id', $run->id)
            ->whereIn('user_id', $selectedUserIds->all())
            ->get();
        ...
        if ($lines->isEmpty()) {
            return response()->json([
                ...
                'error' => [
                    'code' => 'PAYROLL_DISBURSE_NO_EMPLOYEES',
                    'message' => 'Tidak ada payroll line untuk karyawan terpilih.',
                ],
            ], 422);
        }
        ...
        $eligibleSelected = $selectedUserIds
            ->filter(fn (int $userId): bool => ((float) ($netByUser[$userId] ?? 0.0)) > 0)
            ->values();

        if ($eligibleSelected->isEmpty()) {
            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'PAYROLL_DISBURSE_NO_EMPLOYEES',
                    'message' => 'Tidak ada karyawan eligible (net pay harus positif).',
                ],
            ], 422);
        }

        $totalAmount = round((float) $eligibleSelected->sum(fn (int $userId): float => (float) ($netByUser[$userId] ?? 0.0)), 2);
        if ($totalAmount <= 0) {
            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'PAYROLL_AMOUNT_INVALID',
                    'message' => 'Total pembayaran payroll tidak valid.',
                ],
            ], 422);
        }

        $callbackToken = (string) Str::random(40);
        $selectedCsv = $eligibleSelected->implode(',');
        ...
        $preferXendit = $this->shouldUseXenditCheckout($gatewayMode, $request);
        ...
        if ($preferXendit) {
            try {
                ...
                $result = $xenditService->createInvoice([
                    'external_id' => sprintf('payroll-run-%d-%s', $run->id, Str::lower(Str::random(10))),
                    'amount' => (int) round($totalAmount),
                    'description' => sprintf('Payroll run %d funding', $run->id),
                    'customer_name' => (string) ($request->user()?->name ?? 'Payroll Admin'),
                    ...
                    'metadata' => [
                        ...
                        'selected_user_ids' => $eligibleSelected->all(),
                    ],
                ]);

                if ($result && ! empty($result['invoice_url']) && ! empty($result['id'])) {
                    $provider = 'xendit';
                    $hostedCheckoutUrl = (string) $result['invoice_url'];
                    $xenditInvoiceId = (string) $result['id'];
                }
        ...
        if ($hostedCheckoutUrl === '') {
            if (! $allowMock) {
                return response()->json([
                    ...
                    'error' => [
                        ...
                        'message' => 'Xendit payroll checkout is not configured and mock checkout is disabled.',
                    ],
                ], 502);
            }
            ...
            $hostedCheckoutUrl = url('/mock-hosted-payment.html').'?'.http_build_query([
                'payment_uuid' => null,
                'invoice_uuid' => 'payroll-run-'.$run->id,
                'invoice_number' => 'PAYROLL-RUN-'.$run->id,
                'amount' => $totalAmount,
                'callback_token' => $callbackToken,
                'success_url' => $successUrl,
                'failure_url' => $failureUrl,
        ...
        cache()->put("payroll_gateway_checkout:{$run->id}:{$callbackToken}", [
            'selected_user_ids' => $eligibleSelected->values()->all(),
            'provider' => $provider,
        ...
        return response()->json([
            ...
            'data' => [
                'runId' => $run->id,
                'selectedUserIds' => $eligibleSelected->values()->all(),
    ...
    public function confirmMockHostedCheckout(Request $request, int $id): JsonResponse
    {
        ...
        $validated = $request->validate([
            'callbackToken' => ['required', 'string', 'min:20', 'max:120'],
            'userIds' => ['required', 'array', 'min:1'],
            'userIds.*' => [function (string $attribute, mixed $value, Closure $fail): void {
                if (! $this->userIdentifierExists($value)) {
                    $fail("The selected {$attribute} is invalid.");
        ...
        $resolvedUserIds = $this->resolveUserIdsFromIdentifiers($validated['userIds'])->values();
        $callbackToken = (string) $validated['callbackToken'];

        $cacheKey = "payroll_gateway_checkout:{$id}:{$callbackToken}";
        $checkoutState = cache()->get($cacheKey);
        if (! is_array($checkoutState)) {
            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'PAYROLL_GATEWAY_SESSION_INVALID',
                    'message' => 'Sesi hosted checkout tidak valid atau sudah kadaluarsa.',
                ],
            ], 422);
        }

        $expectedUserIds = collect($checkoutState['selected_user_ids'] ?? [])->map(fn ($value): int => (int) $value)->sort()->values();
        $actualUserIds = $resolvedUserIds->sort()->values();
        if ($expectedUserIds->all() !== $actualUserIds->all()) {
            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'PAYROLL_GATEWAY_USER_MISMATCH',
                    'message' => 'Daftar user checkout tidak cocok dengan sesi hosted payment.',
                ],
            ], 422);
        }

        $provider = (string) ($checkoutState['provider'] ?? 'mock');
        $xenditInvoiceId = (string) ($checkoutState['xendit_invoice_id'] ?? '');
        if ($provider === 'xendit' && $xenditInvoiceId !== '') {
            try {
                /** @var XenditService $xenditService */
                $xenditService = app(XenditService::class);
                $invoice = $xenditService->getInvoice($xenditInvoiceId);
                $status = strtoupper((string) ($invoice['status'] ?? ''));
                if (! in_array($status, ['SETTLED', 'PAID'], true)) {
                    return response()->json([
                        'success' => false,
                        'error' => [
                            'code' => 'PAYROLL_GATEWAY_UNSETTLED',
                            'message' => 'Pembayaran payroll belum settlement di Xendit.',
                        ],
                    ], 422);
                }
            } catch (\Throwable $exception) {
                return response()->json([
                    'success' => false,
                    'error' => [
                        'code' => 'PAYROLL_GATEWAY_VERIFY_FAILED',
                        'message' => 'Gagal memverifikasi status payment gateway payroll.',
                    ],
                ], 502);
            }
        }

        cache()->put($cacheKey, array_merge($checkoutState, [
        ...
        return response()->json([
            ...
            'data' => [
                'runId' => $id,
                'provider' => $provider,
                'selectedUserIds' => $actualUserIds->values()->all(),
