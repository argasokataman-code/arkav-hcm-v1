(function (window, document) {
  ...
  const PAGE_SIZE = 20;
  const dashboardUtils = window.ArcavSuperAdminDashboardUtils || {};
  const redirectTo = dashboardUtils.redirectTo || function () {};
  const redirectForAccessError = dashboardUtils.redirectForAccessError || function () { return false; };
  const apiRequest = dashboardUtils.apiRequest || function () { return Promise.reject(new Error("Dashboard utils not loaded")); };
  const getApiToken = dashboardUtils.getApiToken || function () { return Promise.reject(new Error("Dashboard utils not loaded")); };
  const esc = dashboardUtils.esc || function (v) { return String(v || ""); };
  const formatCurrency = dashboardUtils.formatCurrency || function (amount) { return String(amount || 0); };
  const formatPercentage = dashboardUtils.formatPercentage || function (value) { return String(value || 0); };
  const formatDate = dashboardUtils.formatDate || function (value) { return String(value || "-"); };
