...
| `/email-settings` | Email Settings | `settings/email-settings.blade.php` display-only config table | `GET /email-settings`, `GET/PUT /v1/hcm/email-settings`, `POST /v1/hcm/email-settings/test-connection`, `GET /v1/hcm/email-settings/mailtrap-status` | **Global HCM Admin** (`hcm.web.global-admin`) | Semua endpoint email settings runtime sekarang **global-only** (`ensureGlobalHcmAdmin`) | Halaman web aktif menampilkan konfigurasi efektif ENV/config; API profile + test-connection tersedia di backend tetapi belum di-wire ke Blade aktif |
