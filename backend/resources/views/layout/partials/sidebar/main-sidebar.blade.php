...
@if ($showTemplateCatalogMenus)
                {{-- RECRUITMENT section removed: template demo catalog not relevant for HCM product --}}
@endif
...
@if ($canSeeMyPayslipMenu)
                        <li>
                            <a href="{{url('payslip')}}" class="{{ Request::is('payslip') ? 'active' : '' }}">
                                <i class="ti ti-file-invoice"></i><span>My Payslip</span>
                            </a>
                        </li>
@endif
                    </ul>
...
@if ($isHcmAdmin && ! $hideTenantOperationalReports)
                        ...
                        <li class="submenu">
                            ...
                            <ul>
                                <li class="submenu submenu-two">
                                    <a href="javascript:void(0);" class="{{ Request::is('profile-settings','company-profile','security-settings','notification-settings','geofences','tax-employees*','taxes','bpjs-governance*','employee-allowance-governance*','spt-masa-pph21*') ? 'active subdrop' : '' }}">General Settings<span class="menu-arrow inside-submenu"></span></a>
                                    <ul>
...
@endif
@if ($showTemplateCatalogMenus)
                {{-- PAGES, AUTHENTICATION, and UI INTERFACE sections removed: template demo catalogs not relevant for HCM product --}}
@endif
