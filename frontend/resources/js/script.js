...
(function () {
	...
	// Upload Image (expects .upload-pic ancestor and .frames preview container)
	...
	function renderFilePreview($input) {
		...
		var $root = $input.closest(".upload-pic");
		if ($root.length === 0) {
			return;
		}
		var $frames = $root.find(".frames");
		if ($frames.length === 0) {
			return;
		}
		$frames.empty();
		for (var i = 0; i < input.files.length; i++) {
			...
			if (file.type && file.type.indexOf("image/") === 0) {
				var $thumb = $("<img>", {
					src: url,
					alt: file.name || "",
					class: "policy-preview-thumb rounded border",
				}).css({ width: "100px", height: "100px", objectFit: "cover" });
				$frames.append($thumb);
			} else {
				$frames.append(
					$("<span>", {
						class: "badge bg-light text-dark border px-2 py-2 text-break",
						text: file.name || "File",
						title: file.name || "",
					})
				);
	...
	$(document).on("change", ".image-sign", function () {
	...
	// Hapus kunci scroll Bootstrap yang tertinggal (body.modal-open / backdrop) agar sidebar & konten bisa scroll lagi
