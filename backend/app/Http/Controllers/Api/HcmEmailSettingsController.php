...
class HcmEmailSettingsController extends Controller
{
    ...
    public function update(Request $request): JsonResponse
    {
        ...
        if ($data['provider'] === 'mailtrap') {
            $accountId = (int) ($data['mailtrap']['accountId'] ?? 0);
            $token = trim((string) (($data['mailtrap']['apiToken'] ?? '')));
            if ($accountId <= 0 || $token === '') {
                return response()->json([
                    'success' => false,
                    'error' => [
                        'code' => 'VALIDATION_ERROR',
                        'message' => 'Mailtrap accountId and apiToken are required when provider is mailtrap.',
                    ],
                ], 422);
            }
        }

        $result = $this->settingsService()->updateProfile($data, $request->user());
    ...
    public function sendCompose(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::info('EMAIL_COMPOSE_ACCEPTED_BY_TRANSPORT', [
                ...
                'transportSource' => $transport['source'] ?? null,
                'transportHost' => $transport['host'] ?? null,
                'note' => 'Accepted by SMTP transport only; final inbox delivery depends on provider and recipient policy.',
            ]);
