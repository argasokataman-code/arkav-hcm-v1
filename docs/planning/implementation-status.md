...
Detail flow per modul sekarang dipisah di `docs/features/*` (mulai dari `docs/features/README.md`). **Web `/pages`:** peta modul HCM + SaaS (`pages.blade.php`, `config/hcm_portal_hub.php`, `build/js/pages-hcm-hub.js`).
...
**Template HCM aktif + role/permission (semua menu ter-wire):** `docs/planning/active-hcm-templates-and-permissions.md` — tabel path web, modul JS, area API, target akses halaman & API; dipakai bersama rule Cursor `role-permissions-with-features` agar setiap development punya pandangan konsisten. **Sidebar SaaS:** submenu **Super Admin** (Companies, Subscriptions, Packages, Domain, Purchase Transaction) mengikuti **`$isHcmAdmin`** di `sidebar.blade.php` / `header.blade.php`; katalog demo tema mengikuti **`$showTemplateCatalogMenus`** (email utama QA).

**Siklus payroll (referensi produk):** `docs/planning/payroll-lifecycle.md` — pre / actual / post payroll dipetakan ke modul Arcav + backlog terarah. **UI `/payroll-run`:** Calculate Draft manual dulu → Export Reconciliation (`payroll_run` / `disburse`) + unduh CSV → Pay via Gateway; detail di `docs/features/payroll-runs/README.md`.

**Knowledge Base:** halaman **`/knowledgebase`** + **`/knowledgebase/category/{slug}`** + **`/knowledgebase/article/{slug}`** — konten operasional admin dari `config/hcm_knowledgebase.php` (beberapa kategori: memulai/onboarding, dashboard, organisasi, cuti/absensi, payroll termasuk overtime/PKWT, pengaturan sistem, pelaporan langkah-demi-langkah, kinerja/goals/training, wilayah, lembur/tiket, SaaS, dukungan); redirect legacy `/knowledgebase-view` / `/knowledgebase-details`. Tes: `KnowledgebaseWebTest`. Dokumentasi: `docs/features/knowledgebase/`.

**Landing Pages (Marketing) — planned (UI) + partial implemented (API):** target halaman landing public (guest) yang menampilkan promosi paket + CTA “Mulai trial/subscribe”. Backend sudah punya endpoint onboarding publik untuk self-serve: **`POST /v1/public/onboarding`** (buat company + owner + subscription; opsional langsung `pending_payment` + invoice draft + kirim email invoice async). Dokumen API: `docs/api/public-onboarding-api.md` + OpenAPI `docs/api/openapi.yaml`. UI landing + form onboarding masih `planned`: `docs/features/landing-pages/`.

**Trial & Billing Dashboard (Company list) — planned (UI) + partial implemented (API):** layar admin-only untuk memantau company/tenant dengan badge status **TRIAL (1 bulan)** vs **ACTIVE/PAID** vs **PENDING_PAYMENT**, menampilkan invoice terakhir + status email invoice (automated). Backend sudah punya endpoint admin-only: **`GET /v1/saas/companies/billing-overview`** (tab `trial|subscribed`, search, pagination) dan aksi email invoice **`POST /v1/saas/invoices/{invoice}/send-email`** yang mencatat log ke `invoice_email_logs`. Dokumen API: `docs/api/saas-billing-overview-api.md` + OpenAPI `docs/api/openapi.yaml`. UI dashboard masih `planned`: `docs/features/trial-billing-dashboard/`.
...
- **Master komponen gaji (API):** `GET/POST /salary-components`, `GET /salary-components/{id}`, `PUT/DELETE /salary-components/{id}` (hanya **hcmAdmin**). Tetap dipakai mesin/seed; **UI admin** aktif di `/salary-component-master`, sementara `/payroll` dan `/payroll-deduction` hanya mengonsumsi master aktif sebagai sumber taut payroll item. Spesifikasi: `docs/api/hcm-salary-components-api.md`. Tes: `HcmSalaryComponentApiTest`.
...
- **Halaman gaji karyawan:** `/employee-salary` (middleware **`hcm.web.admin`**) + `employee-salary-data.js` — daftar kompensasi dari `GET /v1/hcm/employees`, sunting **`baseSalary` / `fixedAllowance`** lewat `PUT /v1/hcm/employees/{id}`, serta assignment payroll item custom per karyawan via `GET/POST/PUT/DELETE /v1/hcm/payroll-item-assignments`; assignment aktif ikut masuk ke draft payroll bulanan. Dokumentasi: `docs/features/employee-salary/README.md`.
...
- **Attendance:** admin timesheets, schedule timing, punch/me/history/stats, koreksi, dll. **Punch karyawan wajib GPS:** `POST /attendance/me/punch` memerlukan `latitude`/`longitude`; disimpan di `attendance_records`; UI `/attendance-employee` memakai **Leaflet + OpenStreetMap** (unpkg) + Geolocation API. Jika browser memblokir geolocation (`denied`), user bisa memilih titik manual di peta sebagai fallback operasional. **Selfie:** sebelum punch tidak ada baris `attendance_records` — UI menampilkan **modal** “Punch masuk diperlukan” (bukan hanya toast) saat tombol selfie diklik tanpa `punchState` in/done atau saat API mengembalikan `ATTENDANCE_NOT_STARTED`; `attendance-data.js` meng-set `data-arcav-selfie-allowed` dari `GET /attendance/me/today`.
...
- **Export reconciliation evidence (gate finansial):** `POST /v1/reconciliation/exports`, `GET /v1/reconciliation/exports`, `GET /v1/reconciliation/exports/{id}/download` (**hcmAdmin**, tenant-scoped). UI payroll/THR/PKWT mengirim `format` + `filterPayload`; backend menerima alias (`format`→`fileFormat`, `filters`→`filterPayload`) dan untuk **`csv`** dapat **auto-generate** `filePath` di `reconciliation/generated/...` bila `filePath` tidak dikirim. Dokumentasi fitur: `docs/features/export-reconciliation/`.
...
- `docs/api/openapi.yaml` disinkronkan bertahap dengan `/v1/hcm` (termasuk promotion/resignation) dan bagian SaaS subscriptions; bila bertentangan, utamakan **`backend/routes/api.php`** dan spesifikasi markdown per fitur.
- **Aturan agen (Cursor + GitHub Copilot):** detail kanonik di **`.cursor/rules/*.mdc`**; ringkasan per glob untuk agen di GitHub di **`.github/instructions/*.instructions.md`**; indeks integrasi dua arah: **`.github/instructions/README.md`**; navigasi root: **`AGENTS.md`**.
...
4. Menjalankan implementasi bertahap **Export Reconciliation** (`docs/features/export-reconciliation/`) untuk mewajibkan evidence export sebelum aksi finansial irreversible (finalize/disburse/post-payroll/mark-paid/verify), dimulai dari payroll run + invoice/payment.
